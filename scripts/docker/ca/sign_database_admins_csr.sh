#!/bin/bash

if [[ ( -z "${CA_ADMIN_USER}" ) || \
      ( -z "${CA_ADMIN_GROUP}" ) || \
      ( -z "${CA_ADMIN_HOME}" ) || \
      ( -z "${CA_KEY}" ) || \
      ( -z "${CA_KEY_PASSWORD}" ) || \
      ( -z "${CA_CRT}" ) || \
      ( -z "${POSTGRESQL_MAJOR_VERSION}" ) || \
      ( -z "${DATABASE_ADMIN_USER}" ) || \
      ( -z "${DATABASE_ADMIN_CSR}" ) || \
      ( -z "${DATABASE_ADMIN_CRT}" ) || \
      ( -z "${DATABASE_ADMIN_CRT_PERMISSIONS}" ) ]]; then
    echo "Error! Some of environment variables are not set!" >&2
    exit 1
fi

sudo \
env \
CA_ADMIN_USER="${CA_ADMIN_USER}" \
CA_ADMIN_GROUP="${CA_ADMIN_GROUP}" \
CA_ADMIN_HOME="${CA_ADMIN_HOME}" \
CA_KEY="${CA_KEY}" \
CA_KEY_PASSWORD="${CA_KEY_PASSWORD}" \
CA_CRT="${CA_CRT}" \
POSTGRESQL_MAJOR_VERSION="${POSTGRESQL_MAJOR_VERSION}" \
DATABASE_ADMIN_USER="${DATABASE_ADMIN_USER}" \
DATABASE_ADMIN_CSR="${DATABASE_ADMIN_CSR}" \
DATABASE_ADMIN_CRT="${DATABASE_ADMIN_CRT}" \
DATABASE_ADMIN_CRT_PERMISSIONS="${DATABASE_ADMIN_CRT_PERMISSIONS}" \
bash << "EOF"

run_or_exit()
{
    "$@"
    local RESULT=$?
    if [[ "${RESULT}" -ne 0 ]]; then
        exit 1
    fi
}

check_ownership()
{
    local TARGET="$1"
    local TARGET_USER="$2"
    local TARGET_GROUP="$3"

    echo "Checking ownership of ${TARGET}"
    local USER=$(stat -c "%U" "${TARGET}")
    local GROUP=$(stat -c "%G" "${TARGET}")
    if [[ ( "${USER}" != "${TARGET_USER}" ) || \
          ( "${GROUP}" != "${TARGET_GROUP}" ) ]]; then
        echo "Changing ownership of ${TARGET}" \
             "to ${TARGET_USER}:${TARGET_GROUP}"
        chown "${TARGET_USER}:${TARGET_GROUP}" "${TARGET}"
    else
        echo "Ownership of ${TARGET}" \
             "is already ${TARGET_USER}:${TARGET_GROUP}, skipping."
    fi
}

check_permissions()
{
    local TARGET="$1"
    local TARGET_PERMISSIONS="$2"

    echo "Checking permissions of ${TARGET}"
    local PERMISSIONS=$(stat -c "%a" "${TARGET}")
    if [[ "${PERMISSIONS}" != "${TARGET_PERMISSIONS}" ]]; then
        echo "Changing permissions of ${TARGET}" \
             "to ${TARGET_PERMISSIONS}"
        chmod "${TARGET_PERMISSIONS}" "${TARGET}"
    else
        echo "Permissions of ${TARGET}" \
             "are already ${TARGET_PERMISSIONS}, skipping."
    fi
}

echo "Running as $(whoami)."
echo "Changing directory to ${CA_ADMIN_HOME}"
cd "${CA_ADMIN_HOME}"

DATABASE_ADMIN_HOME=$(getent passwd "${DATABASE_ADMIN_USER}" | cut -d : -f 6)
DATABASE_DATA_DIRECTORY=\
"${DATABASE_ADMIN_HOME}/${POSTGRESQL_MAJOR_VERSION}/main"

if [[ -f "${DATABASE_DATA_DIRECTORY}/${DATABASE_ADMIN_CSR}" ]]; then
    echo "Moving ${DATABASE_DATA_DIRECTORY}/${DATABASE_ADMIN_CSR}" \
         "to ${CA_ADMIN_HOME}"
    mv -f \
    "${DATABASE_DATA_DIRECTORY}/${DATABASE_ADMIN_CSR}" \
    "${CA_ADMIN_HOME}"
else
    echo "${DATABASE_DATA_DIRECTORY}/${DATABASE_ADMIN_CSR}" \
         "does not exist!" >&2
    exit 1
fi

if [[ ! -f "${DATABASE_ADMIN_CRT}" ]]; then
    echo "Generating ${CA_ADMIN_HOME}/${DATABASE_ADMIN_CRT}" \
         "with ${CA_ADMIN_USER}:${CA_ADMIN_GROUP} ownership."
    openssl x509 \
    -req \
    -in "${DATABASE_ADMIN_CSR}" \
    -sha256 \
    -CAkey "${CA_KEY}" \
    -passin env:CA_KEY_PASSWORD \
    -CA "${CA_CRT}" \
    -CAcreateserial \
    -out "${DATABASE_ADMIN_CRT}" \
    -days 365
else
    echo "${CA_ADMIN_HOME}/${DATABASE_ADMIN_CRT} already exists, skipping."

    check_ownership \
    "${CA_ADMIN_HOME}/${DATABASE_ADMIN_CRT}" \
    "${CA_ADMIN_USER}" \
    "${CA_ADMIN_GROUP}"
fi

check_permissions \
"${CA_ADMIN_HOME}/${DATABASE_ADMIN_CRT}" \
"${DATABASE_ADMIN_CRT_PERMISSIONS}"

echo "Verifying ${CA_ADMIN_HOME}/${DATABASE_ADMIN_CRT}"
run_or_exit openssl verify \
-CAfile "${CA_CRT}" \
"${DATABASE_ADMIN_CRT}"

if [[ -f "${DATABASE_ADMIN_CSR}" ]]; then
    echo "Removing ${CA_ADMIN_HOME}/${DATABASE_ADMIN_CSR}"
    rm "${DATABASE_ADMIN_CSR}"
fi

echo "Moving ${CA_ADMIN_HOME}/${DATABASE_ADMIN_CRT}" \
     "to ${DATABASE_DATA_DIRECTORY}"
mv -f \
"${CA_ADMIN_HOME}/${DATABASE_ADMIN_CRT}" \
"${DATABASE_DATA_DIRECTORY}"

if [[ -f "${CA_CRT}" ]]; then
    echo "Copying ${CA_ADMIN_HOME}/${CA_CRT} to ${DATABASE_DATA_DIRECTORY}"
    cp -p -f "${CA_CRT}" "${DATABASE_DATA_DIRECTORY}"
else
    echo "${CA_ADMIN_HOME}/${CA_CRT} does not exist!" >&2
    exit 1
fi

ls -l "${DATABASE_DATA_DIRECTORY}/${DATABASE_ADMIN_CRT}"
ls -l "${DATABASE_DATA_DIRECTORY}/${CA_CRT}"

echo "Finished running as $(whoami)."

EOF

echo